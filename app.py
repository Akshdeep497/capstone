...
if _VOICE_CAPTURE_AVAILABLE and st.session_state["voice_mode"]:
    # ---- WebRTC processors ----
    class FrameGrabber(VideoTransformerBase):
        def __init__(self):
            self.last_rgb = None  # RGB ndarray
        def transform(self, frame):
            img = frame.to_ndarray(format="bgr24")[:, :, ::-1]  # BGR->RGB
            self.last_rgb = img
            return frame  # no on-screen transform
    ...
    class VADBuffer(AudioProcessorBase):
        ...
        def recv(self, frame):
            ...
            self.last_rms = float(np.sqrt(np.mean(np.square(f)) + 1e-9))
            # Keep mono int16 in ring buffer
            i16 = np.clip(f * 32767.0, -32768, 32767).astype(np.int16)
            self.buffer.extend(i16.tolist())
            return frame

        def get_recent_chunk(self, seconds: float = 1.5):
            """Return last N seconds of audio as int16 mono ndarray, with debounce."""
            now = time.time()
    ...
    # Poll every 1.5s while listening
    st.autorefresh(interval=1500, key="vc_poll")
    ...
    if webrtc_ctx.state.playing:
        ...
        if vad is not None and grab is not None:
            ...
            if vad.last_rms > 0.02:  # tweak if needed
                samples, sr = vad.get_recent_chunk(1.6)
                if samples is not None:
                    # Transcribe chunk with Gemini and look for the hotword
                    try:
                        api_key = st.secrets["GOOGLE_API_KEY"]
                    except Exception:
                        st.error("Add GOOGLE_API_KEY to .streamlit/secrets.toml"); st.stop()
                    ...
                    # Ask Gemini-1.5-flash for robust audio transcription
                    parts = [
                        "Transcribe this audio; return the exact words in lowercase.",
                        {"mime_type": "audio/wav", "data": wav_bytes},
                    ]
                    try:
                        transcript = generate_with_gemini(parts, model_name="gemini-1.5-flash")
                    except Exception:
                        transcript = ""
